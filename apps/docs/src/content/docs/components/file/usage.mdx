...
`describeRejection` turns any rejection into a sentence. `onReject` receives a `FileRejected`, whose
`reason` is always one of `type`, `too-large`, `too-small`, `duplicate`, `max-files` or `invalid` if
